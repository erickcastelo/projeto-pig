...
public class TelaPesquisaEstado extends javax.swing.JFrame {

   private List<Estado> estados = new ArrayList<>();
   private Estado estado = new Estado();
   EstadoDAO dao = new EstadoDAO();
    
    public TelaPesquisaEstado(){
        initComponents();      
    ...
    private void pesquisarEspecifico(){
        try {
            preencherEstado();
            EstadoDAO dao = new EstadoDAO();
            this.estados = dao.pesquisar(this.estado);             
             tabelaEstado.setModel(new TabelaModeloEstado(this.estados));
        } catch (BDException ex) {
            ex.printStackTrace();
        }
    }
        ...
        menuItem[1].addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                int mensagem = JOptionPane.showConfirmDialog(null, "Deseja realmente excluir " +
                        estado.getNome() + "?", null, JOptionPane.YES_NO_OPTION);
    ...
    private void verDetalhes(){
        TelaVerDetalhesEstado telaVerDetalhes = new TelaVerDetalhesEstado(estado);
        telaVerDetalhes.setVisible(true);
    }
    ...
    private void alterar(){
        TelaAlterarEstado alterarEstado = new TelaAlterarEstado(estado);
        alterarEstado.setVisible(true);
    }
    
    private void preencherEstado(){
        this.estado.setNome(campoNome.getText());
    }
    
    private void selecionarEstado(MouseEvent evt){
        int linha = tabelaEstado.rowAtPoint(evt.getPoint());
        
        if(linha >= 0){
            tabelaEstado.setRowSelectionInterval(linha, linha);
            linha = tabelaEstado.getSelectedRow();
            
            this.estado = estados.get(linha);            
        }
    }
   ...
   private void preencherTabela() throws BDException{
       ...
       tabelaEstado.setModel(new TabelaModeloEstado(estados));
    ...
    private void campoCadastrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_campoCadastrarActionPerformed
       TelaCadastroEstado telaEstado = new TelaCadastroEstado(estados);
    ...
    private void tabelaEstadoMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tabelaEstadoMouseReleased
        selecionarEstado(evt);
        realizarAcao(evt);
    }//GEN-LAST:event_tabelaEstadoMouseReleased
